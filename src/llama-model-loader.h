...
struct llama_model_loader {
    ...
    struct llama_tensor_weight {
        ...
        llama_tensor_weight(const llama_file * file, uint16_t idx, const struct gguf_context * gguf_ctx, ggml_tensor * tensor, bool check_data_bounds = true) : idx(idx), tensor(tensor) {
            ...
            if (check_data_bounds && (offs + ggml_nbytes(tensor) < offs || offs + ggml_nbytes(tensor) > file->size())) {
    ...
    bool use_synthetic_weights = false;
    bool check_tensors;
    bool no_alloc;
