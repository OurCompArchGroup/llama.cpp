...
static size_t ggml_ame_packed_q4_0_size(const ggml_tensor * tensor) {
    const int64_t nblocks = tensor->ne[0] / QK4_0;
    return (size_t) ggml_nrows(tensor) * (size_t) nblocks * sizeof(block_q4_0_ame);
}

static const block_q4_0_ame * ame_get_packed_q4_weight(const ggml_tensor * tensor) {
    if (tensor->buffer == nullptr || tensor->buffer->context == nullptr) {
        return nullptr;
    }

    ame_buffer_context * ctx = ame_buffer_ctx(tensor->buffer);
    const auto it = ctx->packed_q4.find(tensor);
    if (it == ctx->packed_q4.end()) {
        return nullptr;
    }
    return static_cast<const block_q4_0_ame *>(it->second.data);
}

static void ame_store_packed_q4_weight(
    ggml_backend_buffer_t buffer,
    const ggml_tensor * tensor,
    const void * data
) {
    ame_buffer_context * ctx = ame_buffer_ctx(buffer);
    auto & entry = ctx->packed_q4[tensor];
    const size_t packed_size = ggml_ame_packed_q4_0_size(tensor);
    ...
    if (entry.data != nullptr) {
        const int64_t nblocks = ggml_nrows(tensor) * (tensor->ne[0] / QK4_0);
        ggml_ame_repack_q4_0(entry.data, data, nblocks);
    }
}

static size_t ame_refresh_packed_q4_weights(ggml_backend_buffer_t buffer) {
    ame_buffer_context * ctx = ame_buffer_ctx(buffer);
    size_t repacked = 0;
...
static void ggml_backend_ame_mul_mat(ggml_compute_params * params, ggml_tensor * dst) {
    ...
    if (src0->type == GGML_TYPE_Q4_0) {
        ...
        const block_q4_0_ame * packed_w = ame_get_packed_q4_weight(src0);
        if (packed_w == nullptr) {
            ame_store_packed_q4_weight(src0->buffer, src0, src0->data);
            packed_w = ame_get_packed_q4_weight(src0);
        }
...
static void ggml_backend_ame_buffer_clear(ggml_backend_buffer_t buffer, uint8_t value) {
    ...
    if (value == 0) {
        const size_t repacked_q8 = ame_refresh_packed_q8_weights(buffer);
        if (repacked_q8 > 0) {
            AME_LOG("buffer_clear: synthesized %zu packed Q8_64 tensors after zero-fill", repacked_q8);
        }
    }
}
