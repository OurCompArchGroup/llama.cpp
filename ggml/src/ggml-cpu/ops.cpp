    ...
    // im2col: [N, IC, IH, IW] => [N, OH, OW, IC*KH*KW]
    {
        ...
        for (int64_t in = 0; in < N; in++) {
            for (int64_t ioh = 0; ioh < OH; ioh++) {
                for (int64_t iow = 0; iow < OW; iow++) {
                    for (int64_t iic = ith; iic < IC; iic += nth) {
                        ggml_bf16_t * dst_data = wdata + (in*OH*OW + ioh*OW + iow)*(IC*KH*KW);
                        ...
                        for (int64_t ikh = 0; ikh < KH; ikh++) {
                            for (int64_t ikw = 0; ikw < KW; ikw++) {
                                ...
                                if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                                    dst_data[iic*(KH*KW) + ikh*KW + ikw] = zero;
                                } else {
                                    dst_data[iic*(KH*KW) + ikh*KW + ikw] = GGML_FP32_TO_BF16(src_data[iih*IW + iiw]);
                                }
                            }
                        }
                    }
...
void ggml_compute_forward_im2col(
    ...
    switch (dst->type) {
        ...
        default:
            {
                GGML_ABORT("unsupported im2col type: %s", ggml_type_name(dst->type));
            }
    }
}
...
// ggml_compute_forward_im2col_3d_bf16
// src0: kernel [OC*IC, KD, KH, KW]
// src1: image [N*IC, ID, IH, IW]
// dst:  result [N*OD, OH, OW, IC * KD * KH * KW]
static void ggml_compute_forward_im2col_3d_bf16(
        const ggml_compute_params * params,
              ggml_tensor * dst) {
        ...
        for (int64_t in = 0; in < N; in++) {
            for (int64_t iod = 0; iod < OD; iod++) {
                for (int64_t ioh = 0; ioh < OH; ioh++) {
                    for (int64_t iow = 0; iow < OW; iow++) {
                        for (int64_t iic = ith; iic < IC; iic += nth) {
                            ggml_bf16_t * dst_data = wdata + (in*OD*OH_OW + iod*OH_OW + ioh*OW + iow)*IC_KD_KH_KW;
                            ...
                            for (int64_t ikd = 0; ikd < KD; ikd++) {
                                for (int64_t ikh = 0; ikh < KH; ikh++) {
                                    for (int64_t ikw = 0; ikw < KW; ikw++) {
                                        ...
                                        if (iid < 0 || iid >= ID || iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                                            dst_data[iic*KD_KH_KW + ikd * KH_KW + ikh*KW + ikw] = zero;
                                        } else {
                                            const float * const s = (const float *) ((const char *)src_data + iid*nb12 + iih*nb11 + iiw*nb10);
                                            dst_data[iic*KD_KH_KW + ikd * KH_KW + ikh*KW + ikw] = GGML_FP32_TO_BF16(*s);
                                        }
                                    }
                                }
                            }
...
void ggml_compute_forward_im2col_3d(
    ...
    switch (dst->type) {
        ...
        case GGML_TYPE_BF16:
            {
                ggml_compute_forward_im2col_3d_bf16(params, dst);
            } break;
        case GGML_TYPE_F32:
            {
        ...
        default:
            {
                GGML_ABORT("unsupported im2col_3d type: %s", ggml_type_name(dst->type));
            }
    }
}
                    ...
                    for (int64_t kx = 0; kx < knl_w; ++kx) {
                        ...
                        if (kernel_type == GGML_TYPE_F32) {
                            ...
                            *(ggml_bf16_t *) element_ptr = GGML_FP32_TO_BF16(src_val);
                            ...
                            GGML_ABORT("unsupported conv_2d kernel type: %s", ggml_type_name(kernel_type));
                        }
                    }
                }
            }
                        ...
                        for (int64_t kx = 0; kx < knl_w; ++kx) {
                            ...
                            if (kernel_type == GGML_TYPE_F32) {
                                ...
                                *(ggml_bf16_t *)element_ptr = GGML_FP32_TO_BF16(src_val);
                                ...
                                GGML_ABORT("unsupported conv_3d kernel type: %s", ggml_type_name(kernel_type));
                            }
                        }
                    }
                }
